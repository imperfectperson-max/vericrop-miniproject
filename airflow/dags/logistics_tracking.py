...
def handle_delayed_shipments(**context):
    ...
    try:
        ...
        for shipment in delayed_shipments:
            ...
            logger.info(f"🚚 Monitoring shipment: {batch_id}")
            logger.info(f"   Location: {location}, Temp: {temperature}°C, ETA: {eta}")

            # Check for issues
            if temperature > 7.0:
                logger.warning(f"🌡️ Temperature alert for {batch_id}: {temperature}°C")

            # Simulate sending status update
            # In production, this would update dashboards or send notifications

        logger.info(f"✅ Processed {len(delayed_shipments)} active shipments")

    except Exception as e:
        logger.error(f"❌ Error handling delayed shipments: {e}")
        raise
...
def check_environmental_compliance(**context):
    ...
    try:
        ...
        if compliance_issues:
            return 'handle_compliance_issues'
        else:
...
def handle_compliance_issues(**context):
    ...
    try:
        ...
        for issue in compliance_issues:
            ...
            logger.error(f"🚨 COMPLIANCE ISSUE: {batch_id} - {issue_type}")
            logger.error(f"   Value: {value}, Allowed: {threshold}")

            # In production: trigger alerts, update product status, etc.

        logger.warning(f"⚠️ Handled {len(compliance_issues)} compliance issues")

    except Exception as e:
        logger.error(f"❌ Error handling compliance issues: {e}")
        raise
...
def generate_logistics_report(**context):
    ...
    try:
        ...
        logger.info(f"📦 Logistics Report: {report}")
        context['task_instance'].xcom_push(key='logistics_report', value=report)

    except Exception as e:
        logger.error(f"❌ Error generating logistics report: {e}")
        raise
