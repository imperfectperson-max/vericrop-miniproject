...
-- Partial index for active batches (performance optimization)
CREATE INDEX idx_batches_active ON batches(status) WHERE status IN ('created', 'in_transit');
...
COMMENT ON COLUMN batches.metadata IS 'Flexible JSONB storage for additional batch information';
